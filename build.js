    ...
    try {
        b.bundle(function(err, buf){
            if (err) {
                ...
                fs.createWriteStream(__dirname + '/taskconnect' + (staticfiles ? '' : '/taskconnect') + '/static/js/bundles/common.js').write(buf);
...
fs.readdir(__dirname + "/taskconnect", function(err, filenames) {
    if (err || !filenames) {
        console.log(err);
        return false;
    }
    ...
    for (var i = 0; i < filenames.length; i++) {
        var module_js = __dirname + "/taskconnect/" + filenames[i] + "/static/js";
        if (fs.existsSync(module_js)) {
            module_paths.push(module_js);
        }
        var module_less = __dirname + "/taskconnect/" + filenames[i] + "/static/less";
        if (fs.existsSync(module_less)) {
            module_paths.push(module_less);
        }
        var bundle_path = __dirname + "/taskconnect/" + filenames[i] + "/static/js/bundle_modules";
        if (fs.existsSync(bundle_path)) {
            var dir_bundles = fs.readdirSync(bundle_path);
            for (var j = 0; j < dir_bundles.length; j++) {
                bundles.push({
                    target_file: __dirname + "/taskconnect" + (staticfiles ? '' :  "/" + filenames[i]) + "/static/js/bundles/" + dir_bundles[j],
                    bundle: bundle_path + "/" + dir_bundles[j],
                    alias: dir_bundles[j].split(".").slice(0,-1).join(".")
                });
            }
            if (dir_bundles.length > 0) {
                if (!fs.existsSync(__dirname + "/taskconnect" + (staticfiles ? '' :  "/" + filenames[i]) + "/static/js/bundles")) {
                    fs.mkdirSync(__dirname + "/taskconnect" + (staticfiles ? '' :  "/" + filenames[i]) + "/static/js/bundles");
                }
            }
        }
    }
    ...
    if (!fs.existsSync(__dirname + '/taskconnect' + (staticfiles ? '' : '/taskconnect') + '/static/js/bundles/')) {
        fs.mkdirSync(__dirname + '/taskconnect' + (staticfiles ? '' : '/taskconnect') + '/static/js/bundles/');
    }
